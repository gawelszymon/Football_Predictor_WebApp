...
def get_player_info(player_url):
    ...
    if response.status_code == 200:
        ...
        #rating zawodnika
        rating = get_player_rating(id)




    return name, height, age, league, country_name, position, rating
...
def get_players_hrefs(team, tab):
    ...
    if response.status_code == 200:
        ...
        print("Nie udalo sie polaczyc ze strona")
        return None
...
def get_players_info(team, tab):
    ...
    for href in hrefs:
        name, height, age, league, league_country, position, rating = get_player_info(href)
        print(f"name: {name},  height: {height}, age: {age}, liga: {league}, team: {team},"
              f" league country: {league_country},league rating: {get_league_rating(league,league_country)}, position: {position}, rating: {rating}")
...
# print(get_player_rating(824509))
